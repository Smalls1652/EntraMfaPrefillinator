...
{
    public async Task<PhoneAuthenticationMethod> AddOfficePhoneAuthenticationMethodAsync(string userId, string phoneNumber) => await AddOfficePhoneAuthenticationMethodAsync(userId, phoneNumber, null);
    public async Task<PhoneAuthenticationMethod> AddOfficePhoneAuthenticationMethodAsync(string userId, string phoneNumber, string? parentActivityId)
    {
        using var activity = _activitySource.StartActivity(
            name: "AddOfficePhoneAuthenticationMethodAsync",
        ...
        if (_disableUpdateMethods)
        {
            ...
            throw new GraphClientDryRunException("AddOfficePhoneAuthenticationMethodAsync() was called, but the service is currently configured to disable update methods.");
        }
