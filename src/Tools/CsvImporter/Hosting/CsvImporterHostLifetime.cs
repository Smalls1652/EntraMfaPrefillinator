...
namespace EntraMfaPrefillinator.Tools.CsvImporter.Hosting;

internal sealed class CsvImporterHostLifetime : IHostLifetime, IDisposable
{
    public CsvImporterHostLifetime(IHostEnvironment environment, IHostApplicationLifetime applicationLifetime, ILoggerFactory loggerFactory)
    {
        Environment = environment;
        ApplicationLifetime = applicationLifetime;
        Logger = loggerFactory.CreateLogger<CsvImporterHostLifetime>();
    }
    ...
    public Task WaitForStartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            ((CsvImporterHostLifetime)state!).OnAppStarted();
        }, this);

        _appStoppingRegistration = ApplicationLifetime.ApplicationStopping.Register(state =>
        {
            ((CsvImporterHostLifetime)state!).OnAppStopping();
        }, this);
